...
### Step 7: Scan Memory (for non-trivial tasks)

Before starting work, check if memory files contain relevant prior knowledge.

**Check workspace memory first** (project-specific, in `.agents/memory/`):

- `.agents/memory/decisions-log.md` — Has a related decision been made in THIS project?
- `.agents/memory/common-patterns.md` — Does a proven pattern exist in THIS project?
- `.agents/memory/mistakes-to-avoid.md` — Have we made mistakes in THIS project area?

**Then check global memory** (cross-project, in `antigravity/memory/`):

- `memory/decisions-log.md` — Has a related cross-project decision been made?
- `memory/common-patterns.md` — Does a cross-project pattern apply?
- `memory/mistakes-to-avoid.md` — Is there a cross-project trap to avoid?

If workspace memory files do not exist, check global only.
If a relevant entry exists, reference it in your approach. Do not re-litigate settled decisions or repeat known mistakes.
Skip this step only for trivially simple tasks (single-line fixes, formatting, etc.).

### Step 8: Announce the Configuration
