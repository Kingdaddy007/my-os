...
**Design Authority:** Impeccable owns all visual design execution. For workflow orchestration (which Impeccable workflow to run, when, and how they connect), see `workflow-design-ui.md`. This skill provides the passive design knowledge that runs underneath.

**Motion Stack Policy:**
- Product register (UI micro-interactions, component transitions, dialogs):
  → Use `motion` package (modern Framer Motion). Mandatory. `npm install motion`.
- Brand register (scroll-driven storytelling, cinematic reveals, parallax, 3D):
  → Use GSAP + ScrollTrigger. This is the primary tool for brand surfaces.
- Both may coexist in the same project. They serve different layers.
- Elastic and bounce easing: banned for standard UI. Permitted ONLY for
  entertainment/gaming register or approved playful brand CTAs (max 350ms).

See Layer 2 > Motion for principles.
...
- See `reference/motion-design.md` for easing standards, duration guide, and premium motion materials.

### Interaction Design

- **Every interactive element needs 8 states:** default, hover, focus, active, disabled, loading, error, success. See `reference/interaction-design.md` for the full table.
...
See `reference/design-bans.md` for the current list of absolute bans and the AI slop test.

Match-and-refuse. If you're about to write any banned pattern, rewrite the element with different structure.
