...
Tag every issue with **P0-P3 severity** (consistent with `workflow-impeccable-critique.md`):
- **P0 Blocking**: Prevents task completion — fix immediately
- **P1 Major**: Significant difficulty or WCAG AA violation (including contrast failures) — fix before release
...
At the end of every audit run, append a structured fix list to `contexts/audit-issues.md`:
...
This file is consumed by `/impeccable-polish` alongside `contexts/critique-issues.md` to ensure all identified issues are addressed before shipping. Overwrite the file on each new audit run.
...
1. **Fix the issues** → Run the recommended commands from the action summary (P0 first, then P1, then P2).
