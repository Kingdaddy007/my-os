...
description: Impeccable UI/UX craft workflow
---
...
**MOTION CHECK (Brand Register):**
If `contexts/motion-direction.md` exists:
  → Load `skill-cinematic-motion`
  → Read `motion-direction.md` before writing any component code
  → Plan DOM structure to accommodate:
      - Scroll container wrappers for pinned sections
      - GSAP pin spacers for fixed scroll sequences
      - Section IDs matching the scroll narrative acts (hook, build, climax, resolve)
  → Motion-ready DOM structure is NOT optional for brand register.
     Building components without this plan guarantees refactoring.

If `contexts/motion-direction.md` does NOT exist and this is a brand register project:
  → Do not proceed with craft. Run `workflow-visual-brainstorm.md` Phase 3C first.
...
After craft is complete (feature built and verified in browser):

1. **For quality review** → Run `/impeccable-critique` for a design director review.
2. **For final polish** → Run `/impeccable-polish` for the detail pass before shipping.
3. **If the design feels too safe** → Run `/impeccable-bolder` to amplify impact.
4. **If the design feels too busy** → Run `/impeccable-quieter` to reduce noise.
5. **If the design needs more personality** → Run `/impeccable-delight` for moments of joy.
6. **If the design needs animation** → Run `/impeccable-animate` (reads motion-direction.md if it exists).
7. **If ready to ship** → Deploy.

The golden path: craft → critique → fix → polish → ship.
