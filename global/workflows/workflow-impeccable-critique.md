...
After critique is complete (findings presented, action plan created):

1. **Fix the issues** → Run the recommended commands from the action summary (in priority order).
2. **After all fixes** → Run `/impeccable-polish` for the final detail pass.
3. **Re-run critique** → After fixes, re-run `/impeccable-critique` to see the score improve.
