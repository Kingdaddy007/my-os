---
name: workflow-impeccable-document
description: Impeccable UI/UX document workflow
---
...
## The markdown body: eight sections (exact order)
...
[One paragraph describing the spacing scale, page margins, responsive layout system, grid system, or container constraints.]

### Spacing Vocabulary
- **Scale:** [e.g., "8px-based step scale: 4px, 8px, 16px, 24px, 32px, 48px, 64px"]
...
[One paragraph describing the roundness strategy — e.g. "strict rectilinear layout", "soft rounded corners", "dynamic radius scaling based on container size".]
...
3. **Generate Hex → OKLCH CSS variable scaffold.** When DESIGN.md is created, produce a CSS variable block that bridges the hex tokens (used for Stitch validation) to OKLCH values (used for all CSS implementation):

```css
/* Auto-generated by /impeccable-document from DESIGN.json hex tokens */
/* Hex values are for Stitch validation. OKLCH values are for CSS implementation. */
/* Rule: Never use -hex variables in component CSS. Always use the OKLCH variable. */
:root {
  /* Primary accent */
  --accent-primary-hex: #E6C5B3;       /* Stitch reference only */
  --accent-primary: oklch(0.85 0.08 45); /* Use this in all CSS */

  /* Background */
  --bg-primary-hex: #1C1A1A;
  --bg-primary: oklch(0.12 0.01 20);

  /* Body text */
  --text-primary-hex: #F5F0EB;
  --text-primary: oklch(0.96 0.01 80);

  /* Add one entry per color token in DESIGN.md frontmatter */
}
```

Document this in `skills/ui-ux/SKILL.md` Layer 2 Color section:
*"DESIGN.md uses hex for schema validation. All CSS implementations use the OKLCH equivalents generated by /impeccable-document. Never copy hex from DESIGN.md directly into CSS."*

4. Offer to refine any section: "Want me to revise a section, add component patterns I missed, or adjust the atmosphere language?"
5. **Refresh the session cache.** Run `node {{scripts_path}}/load-context.mjs` one final time so the newly-written DESIGN.md lands in conversation. Subsequent commands in this session will use the fresh version automatically without re-reading.
...
Before interviewing:
1. Check if `contexts/story.md` is present in the active workspace.
2. If `contexts/story.md` is present:
   - Extract color strategy from story.md's `## Visual Direction → Color mood` (e.g., restrained, committed, drenched, full).
   - Extract typography direction from story.md's `## Visual Direction → Typography voice` (e.g., serif display + sans body, mono-forward, etc.).
   - Extract motion energy from story.md's `## Motion Direction → Scroll behavior`.
   - Extract references from story.md's `## Brand Archetype`.
   - Extract anti-references from `PRODUCT.md`'s anti-references or story.md's key messages/objections.
   - Inform the user: "I detected an existing `contexts/story.md` in the workspace. I will use its narrative parameters to seed your `DESIGN.md` automatically, skipping the manual questions. OK?"
3. If `contexts/story.md` is NOT present or the user overrides it:
   - Inform the user: "There's no existing visual system to scan. I'll ask five quick questions to seed a starter DESIGN.md. You can re-run `/impeccable document` once there's code, to capture the real tokens and components. OK?"

If the user prefers to skip, stop. No file.

### Step 2: Five questions (Skip if contexts/story.md was parsed in Step 1)
...
After document is complete (DESIGN.md exists):

1. **For animated/visual projects** → Run `workflow-visual-brainstorm.md` to explore visual directions and create motion-direction.md (Phase 3C).
2. **For building a feature** → Run `/impeccable-shape` to create a task-specific design brief.
3. **For refining existing UI** → Run the appropriate refinement workflow (critique, polish, bolder, etc.).

DESIGN.md is the visual foundation. Every subsequent Impeccable workflow reads it.
