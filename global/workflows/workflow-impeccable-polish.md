...
The required delivery path is: craft → critique → fixes → audit → fixes → polish → final browser verification → deploy. No single path should skip both critique and audit.
...
After polish is complete (feature polished and verified):

1. **If ready to ship** → Deploy.
2. **If the design needs more impact** → Run `/impeccable-bolder` to amplify.
3. **If the design needs less noise** → Run `/impeccable-quieter` to simplify.
4. **If the design needs more personality** → Run `/impeccable-delight` for moments of joy.
5. **If the design needs animation** → Run `/impeccable-animate` (reads motion-direction.md if it exists).
6. **For a final quality check** → Re-run `/impeccable-critique` to verify the score.

Polish is the last step before shipping. After this, deploy.
