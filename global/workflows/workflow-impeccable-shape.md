---
name: workflow-impeccable-shape
description: Impeccable UI/UX shape workflow
---
...
After shape is complete (design brief confirmed):

1. **For full build loop** → Run `/impeccable-craft` (shape → visual direction → build → browser inspection → critique-and-fix).
2. **For manual implementation** → Use the confirmed brief as a guide. Load the recommended reference files from item 9.
3. **If the brief includes animation** → Load `skill-cinematic-motion` and check if motion-direction.md exists. If not, create it during build.
