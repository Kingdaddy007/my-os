---
name: workflow-impeccable-teach
description: Impeccable UI/UX teach workflow
---

# Teach Flow

Gathers strategic design context for a project and writes **PRODUCT.md** at the project root:
...
For the visual design system (DESIGN.md), run `/impeccable-document` after teach is complete.
...
After teach is complete (PRODUCT.md exists):

1. **If DESIGN.md is missing** → Run `/impeccable-document` to capture the visual system.
2. **If DESIGN.md exists** → Proceed to visual exploration:
   - For animated/visual projects → Run `workflow-visual-brainstorm.md` (Phase 3C for motion direction)
   - For building a feature → Run `/impeccable-shape` to create a task-specific design brief
   - For refining existing UI → Run the appropriate refinement workflow (critique, polish, bolder, etc.)

**Do not skip DESIGN.md.** Every other Impeccable workflow reads it. Without it, visual decisions are unanchored.
