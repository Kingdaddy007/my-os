...
- [REQUIRED] Load [debug-report.md](file:///c:/Users/Oviks/antigravitygold/templates/debug-report.md)
- Follow the structure and guidance in the template exactly to record the bug investigation.

---

### STEP 8 — POST-FIX (FOR SIGNIFICANT BUGS)
...
4. Update `memory/mistakes-to-avoid.md` if this is a recurring pattern
5. Update `memory/common-patterns.md` if the fix reveals a reusable pattern
6. Update relevant context files if the bug reveals a gap in documented conventions or domain rules
